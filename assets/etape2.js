(function () {
  ...
  async function rendreLigne(ligne) {
    ...
    div.innerHTML = `
      ...
      <div class="k2-ligne-info" style="flex-basis:100%;">
        ...
        <div style="margin-top:10px;">
          ...
          <div class="k2-ligne-montant-bloc">
            <button type="button" class="k2-btn-rond" data-action="moins">${window.KADOSK_ICONE("minus")}</button>
            <input type="number" class="k2-montant-input" data-role="montant" value="${ligne.montant}" min="${min}" ${max !== Infinity ? 'max="' + max + '"' : ""} />
    ...
    const input = div.querySelector('[data-role="montant"]');
    ...
    div.querySelector('[data-action="moins"]').addEventListener("click", () => majMontant(Number(input.value) - PAS_MONTANT_DEFAUT));
    div.querySelector('[data-action="plus"]').addEventListener("click", () => majMontant(Number(input.value) + PAS_MONTANT_DEFAUT));
